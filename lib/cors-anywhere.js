...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originWhitelist: ['http://fightstream.is'],            // If non-empty, requests not from an origin in this list will be blocked.
  ...
  return function(req, res) {
    ...
    if (location.host === 'crossdomain.xml') {
      ...
      res.end('<?xml version="1.0"?><!DOCTYPE cross-domain-policy SYSTEM "http://www.macromedia.com/xml/dtds/cross-domain-policy.dtd"><cross-domain-policy><allow-access-from domain="http://fightstream.is" /></cross-domain-policy>');
      return;
    }
